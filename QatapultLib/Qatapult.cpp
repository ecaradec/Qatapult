...
void Qatapult::init() {
    ...
    addSource(L"User", new UserSource(L"databases\\user.json"));
...
LRESULT Qatapult::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {   
    ...
    else if(msg == WM_CURRENTVERSION)
    {
        ...
        __int64 currentversion=_atoi64(getFileContent(L"currentversion"));
    ...
    else if(msg == WM_PUSHRESULT)
    {
        ...
        if(m_request==p->request) {
            ...
            //
            std::vector<Rule *> activerules;
            getActiveRules(p->pane,m_args,activerules);

            // collect all active types           
            std::map<CString,Type> activetypes;            
            for(uint i=0;i<activerules.size();i++)
                if(activerules[i]->m_types.size()>p->pane)
                    activetypes[activerules[i]->m_types[p->pane].m_type]=activerules[i]->m_types[p->pane];
            ...
            for(KVObject o=p->pack.root().first(); o!=p->pack.root().end(); o=o.next()) {
                ...
                std::map<CString,Type>::iterator it;
                for(it=activetypes.begin(); it!=activetypes.end(); it++) {
                    if(it->second.m_predicate.match(&obj)) {
                        break;
                    }
                }
                if(it==activetypes.end()) continue;
